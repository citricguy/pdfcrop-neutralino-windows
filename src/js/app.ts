/**
 * pdfcrop Desktop Application
 * Neutralinojs Windows wrapper for the pdfcrop WebAssembly PDF cropper
 *
 * Based on pdfcrop.github.io — modified for native desktop save dialog
 */

// Inter variable font (bundled for offline use)
...
// Neutralino global type declaration
declare global {
    interface Window {
        Neutralino?: {
            init: () => void;
...
// Detect if running inside Neutralinojs
const isNeutralino = (): boolean => typeof window.NL_VERSION !== 'undefined';
...
async function initialize() {
    console.log('Initializing pdfcrop desktop app...');

    // Initialize Neutralino if available
    if (isNeutralino() && window.Neutralino) {
        window.Neutralino.init();
        console.log('Neutralino initialized, version:', window.NL_VERSION);

        // Handle app window close event
        window.Neutralino.events.on('windowClose', () => {
            window.Neutralino!.app.exit(0);
        });
    }
...
function setupEventListeners() {
    ...
    dropZone.addEventListener('drop', async (e) => {
        ...
        if (files && files.length > 0 && files[0].type === 'application/pdf') {
            ...
            // Try by extension for systems that don't set MIME type on drag
            const file = files[0];
...
async function handleCrop(): Promise<void> {
    ...
    try {
        ...
        // Save the result using native dialog or browser download
        await savePDF(croppedPDF, outputFilename);
...
/**
 * Save PDF — uses Neutralino native save dialog when in desktop app,
 * falls back to browser download otherwise.
 */
async function savePDF(uint8Array: Uint8Array, filename: string): Promise<void> {
    if (isNeutralino() && window.Neutralino) {
        // Use native OS save dialog
        try {
            ...
            if (savePath) {
                ...
                await window.Neutralino.filesystem.writeBinaryFile(finalPath, uint8Array.buffer as ArrayBuffer);
            ...
            downloadPDFBrowser(uint8Array, filename);
        }
    } else {
...
/**
 * Browser-based PDF download (fallback when not in Neutralino)
 */
function downloadPDFBrowser(uint8Array: Uint8Array, filename: string): void {
